            ...
            try
            {
                ...
                foreach (var product in products)
                {
                    ...
                    if (!String.IsNullOrEmpty(nutritionalInformationXpathLocal))
                        nutrients = scraper.ProcessTable(scraper.GetChildrenNodes(nutritionalInformationXpathLocal)[product.NutrientTableIndex], product.NutrientTableWidth, item.NutrientTableStartIndex);
